					...
					<div class="row">
						...
						<form action="ajax/graba-pedido-campana.php" method="post" accept-charset="utf-8" id="agrega-pedido" <?php echo $hide1; ?>>
							...
							<div class="form-group">
								...
								<select class="form-control" name="ptdGra" required id="ptdGra3" data-formato="<?php echo $formato; ?>" disabled>
									<option value=""><? if($paisID==7){ ?>Selecionar<? }else{ ?>Seleccione<? } ?></option>
									<?php
										$formID = $r['formID'];
						...
						<form action="ajax/graba-pedido.php" method="post" accept-charset="utf-8" id="agrega-pedido" <?php echo $hide1; ?>>
							...
							<div class="form-group">
								<label for="ptdGra">Instore:</label>
								<select class="form-control" name="ptdGra" required id="ptdGra3" data-formato="<?php echo $formato; ?>">
									<option value=""><? if($paisID==7){ ?>Selecionar<? }else{ ?>Seleccione<? } ?></option>
									<?
									$tema = $db->rawQuery('select * from instores_v3 where formID = '.$formato.' and insEst = 0 order by insNomGen');
									if($tema){
										foreach ($tema as $t) {
											$insID 		= $t['insID'];
											$instore 	= $t['insNomGen'].' - '.get_instore_nom_x_pais_v3($paisID, $formato, $insID);
						...
						<? if($_GET['ptID'] && $_GET['ptdItem'] && $ptdGraOp > 0){?>
							<div class="form-group" id="opcionesgra">
								...
								<select class="form-control" name="ptdGraOp" id="ptdGraOp3" required>
									<?
									$tema = $db->rawQuery('select * from instores_opciones_v3 where  formID = '.$formato.' and insID = '.$ptdGra.' and insOPEst = 0');
									if($tema){
										foreach ($tema as $t) {
									?>							
